...
if __name__ == "__main__":
    ...
    lr_input,hr_target,lr_input_tensor,hr_target_tensor = load_test_data(parsed_args.test_data_name,timescale_factor=4,num_snapshot = 100,in_channel=3,upscale_factor=4)
    ...
    pred_no = get_prediction(model,lr_input_tensor[0:1],task_dt = args.task_dt,n_snapshots = 100,ode_step=args.ode_step)
...
# Usage
    initial_input = lr_input_tensor[0:1]
    all_predictions = forward_predictions(model, initial_input, n_steps=20, args=args)
    RFNE,MAE,MSE = get_metric_RFNE(all_predictions,hr_target_tensor[0:1])
    RFNE2,MAE2,MSE2 = get_metric_RFNE(pred_no,hr_target_tensor[0:1])

    print(RFNE.shape)
    print(RFNE.mean(axis =(0,2)))
    print(RFNE2.mean(axis =(0,2)))
