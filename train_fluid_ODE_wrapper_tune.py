...
def validation(args,model, val1_loader,val2_loader,device):
    ...
    ax[0,0].imshow(inputs.detach().cpu().numpy()[0,0,...],cmap=cmocean.cm.balance)
    ax[0,0].set_title("Input")
    ax[0,1].imshow(out.detach().cpu().numpy()[0,0,0,...],cmap=cmocean.cm.balance)
    ax[0,1].set_title("Output t =0 ")
    ax[0,2].imshow(target.detach().cpu().numpy()[0,0,0,...],cmap=cmocean.cm.balance)
    ax[0,2].set_title("Target t =0 ")
    ax[1,0].imshow(inputs.detach().cpu().numpy()[0,0,...],cmap=cmocean.cm.balance)
    ax[1,0].set_title("Input")
    ax[1,1].imshow(out.detach().cpu().numpy()[0,-1,0,...],cmap=cmocean.cm.balance)
    ax[1,1].set_title("Output t = -1 ")
    ax[1,2].imshow(target.detach().cpu().numpy()[0,-1,0,...],cmap=cmocean.cm.balance)
    ax[1,2].set_title("Target t = -1 ")
    run['val1 visualization'].append(fig)
    target_loss2 = 0 
    input_loss2 = 0
    RFNE2_loss = 0
    psnr2_loss = 0
    ...
    ax[1,2].imshow(target.detach().cpu().numpy()[0,-1,0,...],cmap=cmocean.cm.balance)
    ax[1,2].set_title("Target t = -1 ")
    run['val2 visualization'].append(fig)
...
def train(args,model, trainloader, val1_loader,val2_loader, optimizer,device,savedpath):
    ...
    for epoch in range(args.epochs):
        ...
        if epoch %10 == 0:
            ...
            ax[0,0].imshow(inputs.detach().cpu().numpy()[0,0,...],cmap=cmocean.cm.balance)
            ax[0,0].set_title("Input")
            ax[0,1].imshow(out_t.detach().cpu().numpy()[0,0,0,...],cmap=cmocean.cm.balance)
            ax[0,1].set_title("Output t =0 ")
            ax[0,2].imshow(target.detach().cpu().numpy()[0,0,0,...],cmap=cmocean.cm.balance)
            ax[0,2].set_title("Target t =0 ")
            ax[1,0].imshow(inputs.detach().cpu().numpy()[0,0,...],cmap=cmocean.cm.balance)
            ax[1,0].set_title("Input")
            ax[1,1].imshow(out_t.detach().cpu().numpy()[0,-1,0,...],cmap=cmocean.cm.balance)
            ax[1,1].set_title("Output t = -1 ")
            ax[1,2].imshow(target.detach().cpu().numpy()[0,-1,0,...],cmap=cmocean.cm.balance)
            ax[1,2].set_title("Target t = -1 ")
            run['training visualization'].append(fig)
