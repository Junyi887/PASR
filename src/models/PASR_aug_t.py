...
class NODE(nn.Module):
    def __init__(self, in_dim, out_dim=None, num_layers=4, kernel_size=3, padding=1, activation_fn=nn.Tanh, ode_method="Euler",aug_dim = 5):
        ...
        for i in range(num_layers):
            layers.append(nn.Conv2d(in_dim +aug_dim if i == 0 else out_dim, out_dim, kernel_size, stride=1, padding=padding))
    ...
    def forward(self, x, t,ode_step=1, task_dt=1.0):
        h = task_dt / ode_step
        t_0,t_1 = t[0],t[1]
        t_inside = torch.linspace(t[0], t[1], ode_step,device=x.device,type=x.dtype)
        if self.int_method == "Euler":
            for i in range(ode_step):
                x = torch.cat(x, t_inside[i]*np.ones(x.shape[0],self.aug_dim,x.shape[-2],x.shape[-1]),dim=1)
                x = x + h*self.model(x) 
...
class PASR_aug_t(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=3,
                 ...
                 ,mean = [0],std = [1],  
                 ode_method = "Euler",num_ode_layers = 4,time_update = "NODE",aug_dim = 5,ode_kernel_size = 3,ode_padding = 1, **kwargs):
        super(PASR_aug_t, self).__init__()
        ...
        if self.time_update == "NODE":
            self.ode = NODE(in_dim=embed_dim,aug_dim=aug_dim,ode_method = ode_method,num_layers = num_ode_layers,kernel_size= ode_kernel_size,padding=ode_padding)
        else: 
    ...
    def forward(self, x,n_snapshots = 1,task_dt = 0.01,ode_step = 1,time_evol = True):
        ...
        t = torch.linspace(0,1,n_snapshots+1,device = x.device,dtype = x.dtype)  
        if self.upsampler == 'pixelshuffle':
