...
parser.add_argument('--num_depth',type =int, default= 2)
...
if __name__ == "__main__":
    ...
    depth = [args.depth]*args.num_depth
    num_heads = [args.num_heads]*args.num_depth
    model_list = {
            "PASR_ODE_small":PASR_ODE(upscale=args.scale_factor, in_chans=args.in_channels, img_size=(height,width), window_size=window_size, depths=depth, embed_dim=60, num_heads=num_heads, mlp_ratio=2, upsampler=args.upsampler, resi_conv='1conv',mean=mean,std=std,num_ode_layers = args.ode_layer,ode_method = args.ode_method,ode_kernel_size = args.ode_kernel,ode_padding = args.ode_padding,aug_dim_t=args.aug_dim_t),
    }
